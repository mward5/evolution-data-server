...
static const gchar *
nss_error_to_string (gint errorcode)
{
#define cs(a,b) case a: return b;

	switch (errorcode) {
	cs (SEC_ERROR_IO, _("An I/O error occurred during security authorization."))
	cs (SEC_ERROR_LIBRARY_FAILURE, _("security library failure."))
	cs (SEC_ERROR_BAD_DATA, _("security library: received bad data."))
	cs (SEC_ERROR_OUTPUT_LEN, _("security library: output length error."))
	cs (SEC_ERROR_INPUT_LEN, _("security library has experienced an input length error."))
	cs (SEC_ERROR_INVALID_ARGS, _("security library: invalid arguments."))
	cs (SEC_ERROR_INVALID_ALGORITHM, _("security library: invalid algorithm."))
	cs (SEC_ERROR_INVALID_AVA, _("security library: invalid AVA."))
	cs (SEC_ERROR_INVALID_TIME, _("Improperly formatted time string."))
	cs (SEC_ERROR_BAD_DER, _("security library: improperly formatted DER-encoded message."))
	cs (SEC_ERROR_BAD_SIGNATURE, _("Peer's certificate has an invalid signature."))
	cs (SEC_ERROR_EXPIRED_CERTIFICATE, _("Peer's Certificate has expired."))
	cs (SEC_ERROR_REVOKED_CERTIFICATE, _("Peer's Certificate has been revoked."))
	cs (SEC_ERROR_UNKNOWN_ISSUER, _("Peer's Certificate issuer is not recognized."))
	cs (SEC_ERROR_BAD_KEY, _("Peer's public key is invalid."))
	cs (SEC_ERROR_BAD_PASSWORD, _("The security password entered is incorrect."))
	cs (SEC_ERROR_RETRY_PASSWORD, _("New password entered incorrectly.  Please try again."))
	cs (SEC_ERROR_NO_NODELOCK, _("security library: no nodelock."))
	cs (SEC_ERROR_BAD_DATABASE, _("security library: bad database."))
	cs (SEC_ERROR_NO_MEMORY, _("security library: memory allocation failure."))
	cs (SEC_ERROR_UNTRUSTED_ISSUER, _("Peer's certificate issuer has been marked as not trusted by the user."))
	cs (SEC_ERROR_UNTRUSTED_CERT, _("Peer's certificate has been marked as not trusted by the user."))
	cs (SEC_ERROR_DUPLICATE_CERT, _("Certificate already exists in your database."))
	cs (SEC_ERROR_DUPLICATE_CERT_NAME, _("Downloaded certificate's name duplicates one already in your database."))
	cs (SEC_ERROR_ADDING_CERT, _("Error adding certificate to database."))
	cs (SEC_ERROR_FILING_KEY, _("Error refiling the key for this certificate."))
	cs (SEC_ERROR_NO_KEY, _("The private key for this certificate cannot be found in key database"))
	cs (SEC_ERROR_CERT_VALID, _("This certificate is valid."))
	cs (SEC_ERROR_CERT_NOT_VALID, _("This certificate is not valid."))
	cs (SEC_ERROR_CERT_NO_RESPONSE, _("Cert Library: No Response"))
	cs (SEC_ERROR_EXPIRED_ISSUER_CERTIFICATE, _("The certificate issuer's certificate has expired.  Check your system date and time."))
	cs (SEC_ERROR_CRL_EXPIRED, _("The CRL for the certificate's issuer has expired.  Update it or check your system date and time."))
	cs (SEC_ERROR_CRL_BAD_SIGNATURE, _("The CRL for the certificate's issuer has an invalid signature."))
	cs (SEC_ERROR_CRL_INVALID, _("New CRL has an invalid format."))
	cs (SEC_ERROR_EXTENSION_VALUE_INVALID, _("Certificate extension value is invalid."))
	cs (SEC_ERROR_EXTENSION_NOT_FOUND, _("Certificate extension not found."))
	cs (SEC_ERROR_CA_CERT_INVALID, _("Issuer certificate is invalid."))
	cs (SEC_ERROR_PATH_LEN_CONSTRAINT_INVALID, _("Certificate path length constraint is invalid."))
	cs (SEC_ERROR_CERT_USAGES_INVALID, _("Certificate usages field is invalid."))
	cs (SEC_INTERNAL_ONLY, _("**Internal ONLY module**"))
	cs (SEC_ERROR_INVALID_KEY, _("The key does not support the requested operation."))
	cs (SEC_ERROR_UNKNOWN_CRITICAL_EXTENSION, _("Certificate contains unknown critical extension."))
	cs (SEC_ERROR_OLD_CRL, _("New CRL is not later than the current one."))
	cs (SEC_ERROR_NO_EMAIL_CERT, _("Not encrypted or signed: you do not yet have an email certificate."))
	cs (SEC_ERROR_NO_RECIPIENT_CERTS_QUERY, _("Not encrypted: you do not have certificates for each of the recipients."))
	cs (SEC_ERROR_NOT_A_RECIPIENT, _("Cannot decrypt: you are not a recipient, or matching certificate and private key not found."))
	cs (SEC_ERROR_PKCS7_KEYALG_MISMATCH, _("Cannot decrypt: key encryption algorithm does not match your certificate."))
	cs (SEC_ERROR_PKCS7_BAD_SIGNATURE, _("Signature verification failed: no signer found, too many signers found, or improper or corrupted data."))
	cs (SEC_ERROR_UNSUPPORTED_KEYALG, _("Unsupported or unknown key algorithm."))
	cs (SEC_ERROR_DECRYPTION_DISALLOWED, _("Cannot decrypt: encrypted using a disallowed algorithm or key size."))
	cs (XP_SEC_FORTEZZA_BAD_CARD, _("Fortezza card has not been properly initialized.  Please remove it and return it to your issuer."))
	cs (XP_SEC_FORTEZZA_NO_CARD, _("No Fortezza cards Found"))
	cs (XP_SEC_FORTEZZA_NONE_SELECTED, _("No Fortezza card selected"))
	cs (XP_SEC_FORTEZZA_MORE_INFO, _("Please select a personality to get more info on"))
	cs (XP_SEC_FORTEZZA_PERSON_NOT_FOUND, _("Personality not found"))
	cs (XP_SEC_FORTEZZA_NO_MORE_INFO, _("No more information on that Personality"))
	cs (XP_SEC_FORTEZZA_BAD_PIN, _("Invalid Pin"))
	cs (XP_SEC_FORTEZZA_PERSON_ERROR, _("Couldn't initialize Fortezza personalities."))
	cs (SEC_ERROR_NO_KRL, _("No KRL for this site's certificate has been found."))
	cs (SEC_ERROR_KRL_EXPIRED, _("The KRL for this site's certificate has expired."))
	cs (SEC_ERROR_KRL_BAD_SIGNATURE, _("The KRL for this site's certificate has an invalid signature."))
	cs (SEC_ERROR_REVOKED_KEY, _("The key for this site's certificate has been revoked."))
	cs (SEC_ERROR_KRL_INVALID, _("New KRL has an invalid format."))
	cs (SEC_ERROR_NEED_RANDOM, _("security library: need random data."))
	cs (SEC_ERROR_NO_MODULE, _("security library: no security module can perform the requested operation."))
	cs (SEC_ERROR_NO_TOKEN, _("The security card or token does not exist, needs to be initialized, or has been removed."))
	cs (SEC_ERROR_READ_ONLY, _("security library: read-only database."))
	cs (SEC_ERROR_NO_SLOT_SELECTED, _("No slot or token was selected."))
	cs (SEC_ERROR_CERT_NICKNAME_COLLISION, _("A certificate with the same nickname already exists."))
	cs (SEC_ERROR_KEY_NICKNAME_COLLISION, _("A key with the same nickname already exists."))
	cs (SEC_ERROR_SAFE_NOT_CREATED, _("error while creating safe object"))
	cs (SEC_ERROR_BAGGAGE_NOT_CREATED, _("error while creating baggage object"))
	cs (XP_JAVA_REMOVE_PRINCIPAL_ERROR, _("Couldn't remove the principal"))
	cs (XP_JAVA_DELETE_PRIVILEGE_ERROR, _("Couldn't delete the privilege"))
	cs (XP_JAVA_CERT_NOT_EXISTS_ERROR, _("This principal doesn't have a certificate"))
	cs (SEC_ERROR_BAD_EXPORT_ALGORITHM, _("Required algorithm is not allowed."))
	cs (SEC_ERROR_EXPORTING_CERTIFICATES, _("Error attempting to export certificates."))
	cs (SEC_ERROR_IMPORTING_CERTIFICATES, _("Error attempting to import certificates."))
	cs (SEC_ERROR_PKCS12_DECODING_PFX, _("Unable to import.  Decoding error.  File not valid."))
	cs (SEC_ERROR_PKCS12_INVALID_MAC, _("Unable to import.  Invalid MAC.  Incorrect password or corrupt file."))
	cs (SEC_ERROR_PKCS12_UNSUPPORTED_MAC_ALGORITHM, _("Unable to import.  MAC algorithm not supported."))
	cs (SEC_ERROR_PKCS12_UNSUPPORTED_TRANSPORT_MODE, _("Unable to import.  Only password integrity and privacy modes supported."))
	cs (SEC_ERROR_PKCS12_CORRUPT_PFX_STRUCTURE, _("Unable to import.  File structure is corrupt."))
	cs (SEC_ERROR_PKCS12_UNSUPPORTED_PBE_ALGORITHM, _("Unable to import.  Encryption algorithm not supported."))
	cs (SEC_ERROR_PKCS12_UNSUPPORTED_VERSION, _("Unable to import.  File version not supported."))
	cs (SEC_ERROR_PKCS12_PRIVACY_PASSWORD_INCORRECT, _("Unable to import.  Incorrect privacy password."))
	cs (SEC_ERROR_PKCS12_CERT_COLLISION, _("Unable to import.  Same nickname already exists in database."))
	cs (SEC_ERROR_USER_CANCELLED, _("The user pressed cancel."))
	cs (SEC_ERROR_PKCS12_DUPLICATE_DATA, _("Not imported, already in database."))
	cs (SEC_ERROR_MESSAGE_SEND_ABORTED, _("Message not sent."))
	cs (SEC_ERROR_INADEQUATE_KEY_USAGE, _("Certificate key usage inadequate for attempted operation."))
	cs (SEC_ERROR_INADEQUATE_CERT_TYPE, _("Certificate type not approved for application."))
	cs (SEC_ERROR_CERT_ADDR_MISMATCH, _("Address in signing certificate does not match address in message headers."))
	cs (SEC_ERROR_PKCS12_UNABLE_TO_IMPORT_KEY, _("Unable to import.  Error attempting to import private key."))
	cs (SEC_ERROR_PKCS12_IMPORTING_CERT_CHAIN, _("Unable to import.  Error attempting to import certificate chain."))
	cs (SEC_ERROR_PKCS12_UNABLE_TO_LOCATE_OBJECT_BY_NAME, _("Unable to export.  Unable to locate certificate or key by nickname."))
	cs (SEC_ERROR_PKCS12_UNABLE_TO_EXPORT_KEY, _("Unable to export.  Private Key could not be located and exported."))
	cs (SEC_ERROR_PKCS12_UNABLE_TO_WRITE, _("Unable to export.  Unable to write the export file."))
	cs (SEC_ERROR_PKCS12_UNABLE_TO_READ, _("Unable to import.  Unable to read the import file."))
	cs (SEC_ERROR_PKCS12_KEY_DATABASE_NOT_INITIALIZED, _("Unable to export.  Key database corrupt or deleted."))
	cs (SEC_ERROR_KEYGEN_FAIL, _("Unable to generate public/private key pair."))
	cs (SEC_ERROR_INVALID_PASSWORD, _("Password entered is invalid.  Please pick a different one."))
	cs (SEC_ERROR_RETRY_OLD_PASSWORD, _("Old password entered incorrectly.  Please try again."))
	cs (SEC_ERROR_BAD_NICKNAME, _("Certificate nickname already in use."))
	cs (SEC_ERROR_NOT_FORTEZZA_ISSUER, _("Peer FORTEZZA chain has a non-FORTEZZA Certificate."))
	cs (SEC_ERROR_CANNOT_MOVE_SENSITIVE_KEY, _("A sensitive key cannot be moved to the slot where it is needed."))
	cs (SEC_ERROR_JS_INVALID_MODULE_NAME, _("Invalid module name."))
	cs (SEC_ERROR_JS_INVALID_DLL, _("Invalid module path/filename"))
	cs (SEC_ERROR_JS_ADD_MOD_FAILURE, _("Unable to add module"))
	cs (SEC_ERROR_JS_DEL_MOD_FAILURE, _("Unable to delete module"))
	cs (SEC_ERROR_OLD_KRL, _("New KRL is not later than the current one."))
	cs (SEC_ERROR_CKL_CONFLICT, _("New CKL has different issuer than current CKL.  Delete current CKL."))
	cs (SEC_ERROR_CERT_NOT_IN_NAME_SPACE, _("The Certifying Authority for this certificate is not permitted to issue a certificate with this name."))
	cs (SEC_ERROR_KRL_NOT_YET_VALID, _("The key revocation list for this certificate is not yet valid."))
	cs (SEC_ERROR_CRL_NOT_YET_VALID, _("The certificate revocation list for this certificate is not yet valid."))
	cs (SEC_ERROR_UNKNOWN_CERT, _("The requested certificate could not be found."))
	cs (SEC_ERROR_UNKNOWN_SIGNER, _("The signer's certificate could not be found."))
	cs (SEC_ERROR_CERT_BAD_ACCESS_LOCATION,	 _("The location for the certificate status server has invalid format."))
	cs (SEC_ERROR_OCSP_UNKNOWN_RESPONSE_TYPE, _("The OCSP response cannot be fully decoded; it is of an unknown type."))
	cs (SEC_ERROR_OCSP_BAD_HTTP_RESPONSE, _("The OCSP server returned unexpected/invalid HTTP data."))
	cs (SEC_ERROR_OCSP_MALFORMED_REQUEST, _("The OCSP server found the request to be corrupted or improperly formed."))
	cs (SEC_ERROR_OCSP_SERVER_ERROR, _("The OCSP server experienced an internal error."))
	cs (SEC_ERROR_OCSP_TRY_SERVER_LATER, _("The OCSP server suggests trying again later."))
	cs (SEC_ERROR_OCSP_REQUEST_NEEDS_SIG, _("The OCSP server requires a signature on this request."))
	cs (SEC_ERROR_OCSP_UNAUTHORIZED_REQUEST, _("The OCSP server has refused this request as unauthorized."))
	cs (SEC_ERROR_OCSP_UNKNOWN_RESPONSE_STATUS, _("The OCSP server returned an unrecognizable status."))
	cs (SEC_ERROR_OCSP_UNKNOWN_CERT, _("The OCSP server has no status for the certificate."))
	cs (SEC_ERROR_OCSP_NOT_ENABLED, _("You must enable OCSP before performing this operation."))
	cs (SEC_ERROR_OCSP_NO_DEFAULT_RESPONDER, _("You must set the OCSP default responder before performing this operation."))
	cs (SEC_ERROR_OCSP_MALFORMED_RESPONSE, _("The response from the OCSP server was corrupted or improperly formed."))
	cs (SEC_ERROR_OCSP_UNAUTHORIZED_RESPONSE, _("The signer of the OCSP response is not authorized to give status for this certificate."))
	cs (SEC_ERROR_OCSP_FUTURE_RESPONSE, _("The OCSP response is not yet valid (contains a date in the future)."))
	cs (SEC_ERROR_OCSP_OLD_RESPONSE, _("The OCSP response contains out-of-date information."))
	cs (SEC_ERROR_DIGEST_NOT_FOUND, _("The CMS or PKCS #7 Digest was not found in signed message."))
	cs (SEC_ERROR_UNSUPPORTED_MESSAGE_TYPE, _("The CMS or PKCS #7 Message type is unsupported."))
	cs (SEC_ERROR_MODULE_STUCK, _("PKCS #11 module could not be removed because it is still in use."))
	cs (SEC_ERROR_BAD_TEMPLATE, _("Could not decode ASN.1 data. Specified template was invalid."))
	cs (SEC_ERROR_CRL_NOT_FOUND, _("No matching CRL was found."))
	cs (SEC_ERROR_REUSED_ISSUER_AND_SERIAL, _("You are attempting to import a cert with the same issuer/serial as an existing cert, but that is not the same cert."))
	cs (SEC_ERROR_BUSY, _("NSS could not shutdown. Objects are still in use."))
	cs (SEC_ERROR_EXTRA_INPUT, _("DER-encoded message contained extra unused data."))
	cs (SEC_ERROR_UNSUPPORTED_ELLIPTIC_CURVE, _("Unsupported elliptic curve."))
	cs (SEC_ERROR_UNSUPPORTED_EC_POINT_FORM, _("Unsupported elliptic curve point form."))
	cs (SEC_ERROR_UNRECOGNIZED_OID, _("Unrecognized Object Identifier."))
	cs (SEC_ERROR_OCSP_INVALID_SIGNING_CERT, _("Invalid OCSP signing certificate in OCSP response."))
	cs (SEC_ERROR_REVOKED_CERTIFICATE_CRL, _("Certificate is revoked in issuer's certificate revocation list."))
	cs (SEC_ERROR_REVOKED_CERTIFICATE_OCSP, _("Issuer's OCSP responder reports certificate is revoked."))
	cs (SEC_ERROR_CRL_INVALID_VERSION, _("Issuer's Certificate Revocation List has an unknown version number."))
	cs (SEC_ERROR_CRL_V1_CRITICAL_EXTENSION, _("Issuer's V1 Certificate Revocation List has a critical extension."))
	cs (SEC_ERROR_CRL_UNKNOWN_CRITICAL_EXTENSION, _("Issuer's V2 Certificate Revocation List has an unknown critical extension."))
	cs (SEC_ERROR_UNKNOWN_OBJECT_TYPE, _("Unknown object type specified."))
	cs (SEC_ERROR_INCOMPATIBLE_PKCS11, _("PKCS #11 driver violates the spec in an incompatible way."))
	cs (SEC_ERROR_NO_EVENT, _("No new slot event is available at this time."))
	cs (SEC_ERROR_CRL_ALREADY_EXISTS, _("CRL already exists."))
	cs (SEC_ERROR_NOT_INITIALIZED, _("NSS is not initialized."))
	cs (SEC_ERROR_TOKEN_NOT_LOGGED_IN, _("The operation failed because the PKCS#11 token is not logged in."))
	cs (SEC_ERROR_OCSP_RESPONDER_CERT_INVALID, _("Configured OCSP responder's certificate is invalid."))
	cs (SEC_ERROR_OCSP_BAD_SIGNATURE, _("OCSP response has an invalid signature."))
	...
	cs (SEC_ERROR_OUT_OF_SEARCH_LIMITS, _("Cert validation search is out of search limits"))
	cs (SEC_ERROR_INVALID_POLICY_MAPPING, _("Policy mapping contains anypolicy"))
	cs (SEC_ERROR_POLICY_VALIDATION_FAILED, _("Cert chain fails policy validation"))
	cs (SEC_ERROR_UNKNOWN_AIA_LOCATION_TYPE, _("Unknown location type in cert AIA extension"))
	cs (SEC_ERROR_BAD_HTTP_RESPONSE, _("Server returned bad HTTP response"))
	cs (SEC_ERROR_BAD_LDAP_RESPONSE, _("Server returned bad LDAP response"))
	cs (SEC_ERROR_FAILED_TO_ENCODE_DATA, _("Failed to encode data with ASN1 encoder"))
	cs (SEC_ERROR_BAD_INFO_ACCESS_LOCATION, _("Bad information access location in cert extension"))
	cs (SEC_ERROR_LIBPKIX_INTERNAL, _("Libpkix internal error occurred during cert validation."))
	cs (SEC_ERROR_PKCS11_GENERAL_ERROR, _("A PKCS #11 module returned CKR_GENERAL_ERROR, indicating that an unrecoverable error has occurred."))
	cs (SEC_ERROR_PKCS11_FUNCTION_FAILED, _("A PKCS #11 module returned CKR_FUNCTION_FAILED, indicating that the requested function could not be performed.  Trying the same operation again might succeed."))
	cs (SEC_ERROR_PKCS11_DEVICE_ERROR, _("A PKCS #11 module returned CKR_DEVICE_ERROR, indicating that a problem has occurred with the token or slot."))
	#endif

	#if defined (NSS_VMAJOR) && defined (NSS_VMINOR) && (NSS_VMAJOR > 3 || (NSS_VMAJOR == 3 && NSS_VMINOR >= 89))
	cs (SEC_ERROR_EXPIRED_PASSWORD, _("Expired password"))
	cs (SEC_ERROR_LOCKED_PASSWORD, _("Locked password"))
	cs (SEC_ERROR_UNKNOWN_PKCS11_ERROR, _("Unknown PKCS11 error"))
	cs (SEC_ERROR_CERT_SIGNATURE_ALGORITHM_DISABLED, _("Certificate signature algorithm disabled"))
	cs (SEC_ERROR_LEGACY_DATABASE, _("Legacy database"))
	cs (SEC_ERROR_SIGNATURE_ALGORITHM_DISABLED, _("Signature algorithm disabled"))
	cs (SEC_ERROR_ALGORITHM_MISMATCH, _("Algorithm mismatch"))
	#endif
	}
 ...
 * @nss_error_code: an error code, as returned by PORT_GetError()
 *
 ...
 *   with S/MIME support.
 *
 * Returns: (nullable): a localized text description of the @nss_error_code,
 ...
 **/
const gchar *
camel_smime_context_util_nss_error_to_string (gint nss_error_code)
{
	#ifdef ENABLE_SMIME
	return nss_error_to_string (nss_error_code);
	#else
	return NULL;
	#endif
}
